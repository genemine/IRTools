#Paggi J M, Bejerano G. A sequence-based, deep learning model accurately predicts RNA splicing branchpoints[J]. RNA, 2018, 24(12): 1647-1658. doi: 10.1261/rna.066290.118 
...
def write_best_bed(names, preds, oname):
    with open(oname, 'w') as out:
        for name, pos, score in zip(names, np.argmax(preds, axis = 1), np.max(preds, axis = 1)):
            chrom, three, strand = name.split(':')[:3]
            three = int(three)
